...
;--------------------------------------------------------------
;Section 4.2, Exercise 4.29
;I would expect this program to run much more slowly without memoization
...
(define (fib-pair n)
  ...
  (cond ((= n 0) (cons 0 1))
        ((= n 1) (cons 1 1))
        (else (next (fib-pair (- n 1))))))


;--------------------------------------------------------------
;Section 4.2, Exercise 4.31
;See the usage of delayed?-value and definition syntax below



;--------------------------------------------------------------
;Section 4.2, Exercise 4.33

;Make the following modifications to eval-quoted:

(define (eval-quoted exp env)
  (let ((text (text-of-quotation exp)))
    (if (pair? text)
        (eval (list->list-exp text) env)
        text)))


(define  (list->list-exp elements)
  (if (null? elements) ''()
      (list 'cons
            (list 'quote
                  (car elements))
            (list->list-exp (cdr elements)))))


;--------------------------------------------------------------
;Section 4.2, Exercise 4.34

;Make the following modifications to user-print:

(define (print-pair pair)
  (display "(")
  (define (print-iter pair count)
    (let ((car-pair (force-it (fetch-car pair the-global-environment)))
          (cdr-pair (force-it (fetch-cdr pair the-global-environment))))
      (cond ((> count 20) (display "...)"))
            ((null? cdr-pair) (user-print car-pair) (display ")"))
            ((cons-pair? cdr-pair) (user-print car-pair) (display " ") (print-iter cdr-pair (+ count 1)))
            (else (user-print car-pair) (display " . ")  (user-print cdr-pair) (display ")")))))
  (print-iter pair 0))
...
;--------------------------------------------------------------
;Below find the implementation of the lazy evaluator from this section
...
(define (delayed?-value arg param env)
  (cond
    ((not (pair? param)) (actual-value arg env))
    ((eq? (cadr param) 'lazy) (delay-it arg env))
    ((eq? (cadr param) 'lazy-memo) (delay-it-memo arg env))
    (else (error "Don't recognize this argument tag:" (cadr param)))))
...
(define (param-names args)
  (cond ((null? args) '())
        ((pair? (car args)) (cons (caar args) (param-names (cdr args))))
        (else (cons (car args) (param-names (cdr args))))))
  ...
  (cond ((primitive-procedure? procedure)
        ...
        ((compound-procedure? procedure)
         (eval-sequence
          ...
          (extend-environment
           (param-names (procedure-parameters
                         procedure))
           (list-of-delayed?-args
            arguments
