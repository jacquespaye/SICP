...
(#%require "Section-5.2--MetacircularSource.rkt")

;--------------------------------------------------------------
;Section 5.4, Exercise 5.30
;See below for an implementation of the explicit-control-evaluator with the two desired types of
;error handling.

;--------------------------------------------------------------
;Section 5.5, Exercise 5.32
;In order to incorporate the desired optimization, replace ev-application below with the following
...
ev-appl-symbol-op
(assign
 continue (label ev-appl-did-lookup))
(goto (label eval-dispatch))



;--------------------------------------------------------------
;Below find the evaluator implementation
...
(define eceval
  (ds:make-machine
   ...
   '(
     ...
     error-not-pair
     (assign val (const "Attempted to use car/cdr on an object that is not a pair: "))
     (assign argl (op list->string) (reg argl))
     (assign val (op string-append) (reg val) (reg argl))
     (goto (label signal-error))

     error-div-0
     (assign argl (op list->string) (reg argl))
     (assign val (const "Attempt to divide by zero, arguments are: "))
     (assign val (op string-append) (reg val) (reg argl))
     (goto (label signal-error))
     ...
     error-def-conflict
     (restore unev)
     (assign unev (op symbol->string) (reg unev))
     (assign val (const "The following definition conflicts with error codes: "))
     (assign val (op string-append) (reg val) (reg unev))
     (goto (label signal-error))

     error-assign-var
     (assign val (const "Attempted to assign to an unbound variable: "))
     (assign unev (op symbol->string) (reg unev))
     (assign val (op string-append) (reg val) (reg unev))
     (goto (label signal-error))

     error-unbound-var
     (assign val (const "Attempted to access an unbound variable: "))
     (assign exp (op symbol->string) (reg exp))
     (assign val (op string-append) (reg val) (reg exp))
     (goto (label signal-error))   
     ...
     (assign val
             ...
             (reg exp)
             (reg env))
     ;test if variable is unbound
     (test (op eq?) (reg val) (const ERR_UNBOUND))
     (branch (label error-unbound-var))
     (goto (reg continue))
     ...
     (assign val (op apply-primitive-procedure)
             (reg proc)
             (reg argl))
     (test (op eq?) (reg val) (const ERR_DIV_0))
     (branch (label error-div-0))
     (test (op eq?) (reg val) (const ERR_NOT_PAIR))
     (branch (label error-not-pair))
     (restore continue)
     (goto (reg continue))
     ...
     (assign val (op set-variable-value!)
             ...
             (reg val)
             (reg env))
     (test (op eq?) (reg val) (const ERR_UNBOUND_ASSIGN))
     (branch (label error-assign-var))
     (goto (reg continue))
     ...
     (assign unev 
             ...
             (reg exp))
     (save unev)   ; save variable for later
     ;make sure we are not defining an error code
     (assign unev
             (op symbol->string)
             (reg unev))
     (assign unev
     ...
     (test (op equal?) (reg unev) (const "ERR"))
     (branch (label error-def-conflict))
     (restore unev)
     (save unev)
