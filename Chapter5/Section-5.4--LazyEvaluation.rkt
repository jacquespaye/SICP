...
;--------------------------------------------------------------
;Section 5.4, Exercise 5.25

;See below for implementation of the lazy explicit-control evaluator,
;based on the lazy evaluator of 4.2
...
(define eceval
  (ds:make-machine
   ...
   '(
     ...
     ev-appl-force-it
     (save continue)
     (assign continue (label test-thunk))
     (goto (label eval-dispatch))

     test-thunk
     (test (op eval-thunk?) (reg val))
     ...
     (goto (label test-thunk))
     
     force-thunk
     (save env)
     (assign continue (label after-thunk-eval))
     (assign env (op caddr) (reg val))
     (assign exp (op cadr) (reg val))
     (save val)
     (goto (label eval-dispatch))
     after-thunk-eval
     (restore unev)
     (perform (op set-car!) (reg unev) (const eval-thunk))
     (perform (op set-cadr!) (reg unev) (reg val))
     (perform (op set-caddr!) (reg unev) (const ()))
     ...
     (branch (label ev-appl-force-it))
     (goto (label ev-appl-make-thunk))
